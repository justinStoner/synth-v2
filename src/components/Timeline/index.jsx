...
import { DefaultGroupRenderer, DefaultItemRenderer } from './Renderers';
...
export default class Timeline extends React.Component {
  ...
  static propTypes = {
    ...
    BPMe: PropTypes.number,
    SPB: PropTypes.number,
  };
  ...
  setTimeMap(items, startDate, endDate) {
    ...
    const visibleItems = _.filter(items, i => i.end > startDate && i.start < endDate);
    const itemRows = _.groupBy(visibleItems, 'row');
  ...
  setSelection(selections) {
    const newSelection = _.map(selections, s => ({ start: s[0].clone(), end: s[1].clone() }));
  ...
  setUpDragging(canSelect, canDrag, canResize) {
      ...
      this._itemInteractable
        ...
        .on('dragstart', e => {
          ...
          _.forEach(animatedItems, id => {
            const domItem = this._gridDomNode.querySelector("span[data-item-index='" + id + "'");
            if (domItem) {
        ...
        .on('dragmove', e => {
          ...
          const animatedItems = this._gridDomNode.querySelectorAll("span[isDragging='True'") || [];

          const dx = (parseFloat(target.getAttribute('drag-x')) || 0) + e.dx;
          const dy = (parseFloat(target.getAttribute('drag-y')) || 0) + e.dy;
          const selections = [];
          ...
          _.forEach(animatedItems, domItem => {
            ...
            const newPixelOffset = pixToInt(domItem.style.left) + snapDx;
            const newStart = getTimeAtPixel(
              newPixelOffset,
              this.props.startDate,
              this.props.endDate,
              this.getTimelineWidth(),
            ...
            const newEnd = newStart.clone().add(itemDuration);
        ...
        .on('dragend', e => {
          ...
          const newRow = getNearestRowHeight(e.clientX, e.clientY);
          // console.log('To row', newRow);

          const rowChangeDelta = newRow - rowNo;
          // Update time
          const newPixelOffset = pixToInt(e.target.style.left) + (parseFloat(e.target.getAttribute('drag-x')) || 0);
          const newStart = getTimeAtPixel(
            newPixelOffset,
            this.props.startDate,
            this.props.endDate,
          ...
          _.forEach(animatedItems, domItem => {
            ...
            const itemDuration = item.end.diff(item.start);
            const newStart = item.start.clone().add(timeDelta, 'minutes');
            const newEnd = newStart.clone().add(itemDuration);
            item.start = newStart;
            item.end = newEnd;
            if (rowChangeDelta < 0) {
    ...
    if (canResize) {
      this._itemInteractable
        ...
        .on('resizemove', e => {
          const animatedItems = this._gridDomNode.querySelectorAll("span[isResizing='True'") || [];

          let dx = parseFloat(e.target.getAttribute('delta-x')) || 0;
          dx += e.deltaRect.left;

          const dw = e.rect.width - Number(e.target.getAttribute('initialWidth'));
        ...
        .on('resizeend', e => {
          ...
          _.forEach(animatedItems, domItem => {
            const startPixelOffset = pixToInt(domItem.style.left) + dx;
            const { item, rowNo } = this.itemFromElement(domItem);
            ...
            if (isStartTimeChange) {
              ...
              const endPixelOffset = startPixelOffset + pixToInt(domItem.style.width);
              const newEnd = getTimeAtPixel(
                endPixelOffset,
                this.props.startDate,
                this.props.endDate,
                this.getTimelineWidth(),
            ...
            const new_row_height = getMaxOverlappingItems(
    ...
    if (canSelect) {
      this._selectRectangleInteractable
        ...
        .on('dragend', e => {
          ...
          const startOffset = width > 0 ? left : left + width;
          const endOffset = width > 0 ? left + width : left;
          ...
          for (let r = Math.min(topRow, bottomRow); r <= Math.max(topRow, bottomRow); r++) {
            selectedItems.push(
              ..._.filter(this.rowItemMap[r], i => i.start.isBefore(endTime) && i.end.isAfter(startTime))
            );
          }
  ...
  _handleItemRowEvent = (e, itemCallback, rowCallback) => {
    ...
    if (e.target.hasAttribute('data-item-index') || e.target.parentElement.hasAttribute('data-item-index')) {
      const itemKey = e.target.getAttribute('data-item-index') || e.target.parentElement.getAttribute('data-item-index');
      itemCallback && itemCallback(e, Number(itemKey));
    } else {
      const row = e.target.getAttribute('data-row-index');
      ...
      const snappedClickedTime = timeSnap(clickedTime, this.props.snapMinutes * 60);
  ...
  cellRenderer(width) {
    ...
    return ({ columnIndex, key, parent, rowIndex, style }) => {
      ...
      if (itemCol == columnIndex) {
        const itemsInRow = this.rowItemMap[rowIndex];
        ...
        const group = _.find(this.props.groups, g => g.id == rowIndex);
        return (
   ...
   */
  rowHeight({ index }) {
    const rh = this.rowHeightCache[index] ? this.rowHeightCache[index] : 1;
