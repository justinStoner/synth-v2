...
import { timebarFormat as defaultTimebarFormat } from './constants';
...
export default class Timebar extends React.Component {
   ...
   */
  renderTopBar() {
    const res = this.state.resolution.top;
   ...
   */
  renderBottomBar() {
    const res = this.state.resolution.bottom;
  ...
  renderBar(resolution) {
    const { BPMe, end, selectedRanges } = this.props;
    const width = this.props.width - this.props.leftOffset;
    ...
    function _addTimeIncrement(initialOffset, offsetType, stepFunc) {
      let offset = null;
      while (currentDate < end && pixelsLeft > 0) {
        ...
        const label = currentDate.format(resolution.format[labelSize]);
        const isSelected = _.some(selectedRanges, s => (
  ...
  render() {
    ...
    const bottomBarComponent = this.renderBar();
